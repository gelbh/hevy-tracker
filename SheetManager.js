...
class SheetManager {
  ...
  static getOrCreate(sheetName) {
    ...
    if (!sheet) {
      try {
        ...
        throw new ConfigurationError(`Failed to create sheet ${sheetName}: ${error.message}`);
      }
    }
  ...
  formatData(startRow, numRows) {
    try {
      ...
      const range = this.sheet.getRange(startRow, 1, numRows, this.sheet.getLastColumn());
      ...
      handleError(error, 'Formatting data rows');
    }
  }
  ...
  ensureHeaders() {
    if (!this.validateHeaders()) {
      ...
      this.sheet.getRange(1, 1, 1, this.headers.length)
